...
export function FloatingHero() {
  return (
    ...
    >
      ...
      <Floating
        sensitivity={1}
        className="relative z-10 flex flex-1 flex-col justify-end px-6 pt-28 pb-24 md:px-10 md:pb-20"
      >
        {/* ghosted 方 watermark — Fang, drifting at depth */}
        <FloatingElement
          depth={2.2}
          className={`pointer-events-none top-[24%] right-[23%] ${fragmentVis}`}
        >
          <Fade delay={1.2}>
            <span
              aria-hidden
        ...
        <FloatingElement depth={2.4} className={`top-[44%] right-[22%] ${fragmentVis}`}>
          <Fade delay={1.15}>
            <p className={fragmentClass}>NC State University</p>
          </Fade>
        </FloatingElement>
        <FloatingElement depth={3} className={`top-[64%] right-[29%] ${fragmentVis}`}>
          <Fade delay={1.3}>
        ...
        <div className="mx-auto w-full max-w-[1440px]">
          {/* -ml cancels Switzer's left side bearing on the display caps, so
              EMILY/FANG optically align with the mono line and pills below
              instead of sitting ~9-17px inset. In em so it tracks the clamp;
              0.085 splits the hinted bearing across the clamp's range
              (0.081em at 111px, 0.089em at the 192px cap) — under 1px off at
              either end. */}
          <h1 className="-ml-[0.085em] text-[clamp(3.5rem,min(13.5vw,20svh),12rem)] leading-[0.88] font-extralight tracking-[-0.035em] uppercase">
            <Rise delay={0.35}>
      ...
      {/* rotating scroll badge — bottom-right; visible on phones too */}
      <Fade
        delay={1.6}
        className="absolute right-8 bottom-8 z-10 hidden [@media(min-height:560px)]:block"
      >
        <ScrollBadge />
      </Fade>
    </section>
